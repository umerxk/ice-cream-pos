      ...
      <div className="receipt-items">
        {orderData.map((item: any, index: any) => (
          <div key={index} className="receipt-item">
            ...
            <span className="item-size" style={{ marginLeft: 10 }}>{item.itemData.price?.small ? item.itemData.price?.large : item.itemData?.price}</span>
            <span className="item-quantity" style={{ marginLeft: 10 }}>x <span style={{ marginLeft: 5 }}>{item.itemQuantity} = </span> </span>
